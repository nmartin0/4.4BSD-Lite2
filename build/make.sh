...
# licence for this tree's own scripts is not settled yet, and the
# tag is left out entirely rather than written in a sentence, where
# a scanner would read it as a declaration. 4.4BSD-Lite2's terms
...
# 1.6's build.sh generates, which sets the target environment and
# execs make, and OpenBSD's Makefile.cross, whose CROSSENV points
# AR, AS, CC, CPP, LD, RANLIB and the rest at the target's tools.
#
