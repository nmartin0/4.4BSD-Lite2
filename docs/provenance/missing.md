...
## The host yacc, and what it costs

This tree is built with byacc, because bin/sh's grammars use the old
`rule = { action }' form that bison rejects. byacc brings a cost of
its own: what it writes declares `extern int yylex(void);', and
4.4BSD-Lite2's own usr.bin/yacc never declares yylex at all. So
libexec/ftpd/ftpcmd.y, which defined yylex static -- correct under the
yacc this tree ships -- would not compile, and its two sites were
changed to match NetBSD 1.6 and usr.sbin/config.new instead.

The proper fix is to build usr.bin/yacc as a host tool and point YACC
at it, as NetBSD does with nbyacc. Then this tree is generated by its
own tools, ftpcmd.y would stand as Berkeley wrote it, and byacc stops
being a host dependency. It is not done yet because it wants every
yacc file here rebuilt and checked against the result; the kernel work
will want the tree's own config built the same way, which is the
natural moment. The spelling ftpcmd.y now has is right under either
yacc, so nothing has to be undone when that happens.

## Games

Thirty of the forty-four in games/ build. The fourteen that do not are
left alone, being the least load-bearing part of the system and the
most work for each one:

  adventure, monop	build a generator and run it -- setup and
			initdeck -- which a build for another machine
			cannot do. Same shape as bin/sh's mkinit,
			which HOSTCC solved.
  hack, larn, rogue	pass -fwritable-strings, a flag GCC removed
			at 4.0. NetBSD still passed it for rogue as
			late as 1.6, so the era's answer was to keep
			the flag; the modern answer is to fix the
			sources, which is a real change to three
			games.
  backgammon, battlestar	call exit() with no argument.
  gomoku, sail		pass the wrong type to log().
  rain			declares a static function inside another.
